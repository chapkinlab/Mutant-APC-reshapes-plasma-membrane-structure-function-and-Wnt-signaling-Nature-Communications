...
#Import Data
#############################################################
Counts <- read.csv(here("data","raw","2021_Alfredo_mice-count.T.csv"), header=T)
dim(Counts)
symbl <- Counts[,1]
...
#GC = 1, AGC = 2, AfGC = 3
group <- factor(c(1,2,2,1,1,1,3,2,2,3,1,2,2,3,1,1,1,1,2,1,2,3))
#############################################################
...
#removed 23674 rows with all zero counts
...
#removed 15464 rows with all zero counts
design <- model.matrix(~0+group) #GLM model statement
...
write.csv(val0,here("data","AfGC_vs_AGC.csv"))

#compare AGC vs GC 
lrt <- glmLRT(fit, contrast=c(-1,1,0))
out <- topTags(lrt,sort.by = "none", n=nrow(Counts))$table
val0 <- (cbind(out,FC=2^({lrt$table[,"logFC"]})))
write.csv(val0,here("data","AGC_vs_GC.csv"))

#compare AfGC vs GC 
lrt <- glmLRT(fit, contrast=c(-1,0,1))
out <- topTags(lrt,sort.by = "none", n=nrow(Counts))$table
val0 <- (cbind(out,FC=2^({lrt$table[,"logFC"]})))
write.csv(val0,here("data","AfGC_vs_GC.csv"))
